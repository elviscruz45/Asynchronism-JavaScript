...
//llamando al xml httprequest
const XMLHttpRequest=require("xmlhttprequest").XMLHttpRequest;

//llamando API web site address
const API="https://api.escuelajs.co/api/v1";

function fetchData(urlApi,callback){
    //se crea un objeto a partir de la clase xmlhtttprequest
    let xhttp=new XMLHttpRequest();

    //El metodo .open realiza la petición de apertura de comunicación, el metodo puede ser 'GET' o 'POST',
    //luego se envia la URL, si es asincrono (true o false), usuario y contraseña.
        ...
        if (xhttp.readyState===4){
            ...
            if (xhttp.status===200){
                ...
                //si la respuesta de la API no es exitosa se captura el error
                 ...
                 //se inicializa un objeto de tipo Error donde se le envian como argumentos un mensaje de error
                 // y la URL de la API para conocer en dónde se produjo el error
...
// Se invoca el metodo fetchData() pasandole como argumentos la varible API concatenada con
// la cadena 'products' para acceder a la URL de la API deseada,
// y una función anónima que recibe 2 parámetros (un objeto de error y un arreglo que
// almacena todos los objetos traidos por la API).
fetchData(`${API}/products`,function(error1,data1){
    ...
    fetchData(`${API}/products/${data1[2].id}`,function(error2,data2){
        ...
        //en este caso puntual se hace uso de Optional Caining el cual hace una evalucación de 
        //las propiedades de un objeto y en vez de arrojar un error devuelve undefined en caso que la propiedad no exista o sea null.
